...
class WallFollower:
    ...
    def __init__(self, direction):
  	...
  	self.pub = rospy.Publisher("/vesc/low_level/ackermann_cmd_mux/input/navigation",
                                   AckermannDriveStamped, queue_size=1)
	self.pub1 = rospy.Publisher("/vesc/low_level/ackermann_cmd_mux/input/n",
                                   AckermannDriveStamped, queue_size=1)
        self.sub = rospy.Subscriber(
            "/scan1", LaserScan, self.lidarCB, queue_size=1)
    ...
    def drive_straight(self):
        while not rospy.is_shutdown():
            ...
            self.pub1.publish(drive_msg_stamped)
            # don't spin too fast
    ...
    def apply_control(self):
        while not rospy.is_shutdown():
            if self.control is None:
                ...
                drive_msg = AckermannDrive()
                drive_msg.speed = 1.1
                drive_msg.steering_angle = -0.0162
                drive_msg.acceleration = 0
                drive_msg.jerk = 0
                drive_msg.steering_angle_velocity = 0
                drive_msg_stamped.drive = drive_msg
