...
public:
    WallFollow():
            ...
            lidar_sub_(node_handle_.subscribe("scan1", 5, &WallFollow::scan_callback, this)),
            drive_pub_(node_handle_.advertise<ackermann_msgs::AckermannDriveStamped>("vesc/high_level/ackermann_cmd_mux/input/nav_0", 5))
    {
    ...
    void control_steering()
    {
        ...
        if(steering_angle > 0.34)
        {
            steering_angle = 0.34;
        }
        else if(steering_angle < -0.34)
        {
            steering_angle = -0.34;
        }
